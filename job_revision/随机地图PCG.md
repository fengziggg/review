...
- 1D/2D/...
  - 1D到2D是靠插值插出来，以此类推  
  - 平滑是考缓释曲线实现(3t^2-2t^3, ...)，单纯线性插值出来的东西还是线性的(主要是负责插值而不是平滑)
    
- 类型：
  - 1. Value噪声：晶格和代表晶格的高度值，直接缓释插值
       
  - 2. 柏林噪声：晶格和顶点上的梯度，点乘后，缓释，插值  
    > 1. **梯度不是高度值**，一元函数梯度就是一个x，**不是导数**，可以看作一个(x, 0)方向的向量  
    > 2. 二元函数梯度就是一个xz平面的向量用于指示方向  
    > 3. **柏林噪声不是基于高度来插值的**，所以如果拟合某些高度是不可能的，而是相当于给了微分，然后做出一个积分高度图！！  
    > 4. 不然在顶点插值会发现顶点的插值居然是其他三个点的期望，自己的插出来的值是0  

  - 3. Simplex，对晶格的优化(单形，最小可铺满全体空间的多边形，2D是三角形，计算归属单形可以缩放倾斜为四边形)  
  
  - 4. Woley噪声，维诺图形式，遍历的时候聚类挑选最小距离归属

分形噪声：
 - 控制[整幅/振频]的多个噪声叠加图，一般振幅/振频的变化有递归性
