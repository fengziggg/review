...
- 业务
  ...
  - 地图
    ...
    - 渲染TileMap
      ...
      - 1. 基础方案：TileMap内部实心，边缘包边，原生的直接跟Tile一对一实例化对象，但有性能瓶颈：
        > 大量Tile实例即使进入渲染管线后视口外的会被剔除，但引擎层面还有脚本层面还是要维持这些对象的，引擎层面节点树估计还会更新(❓❓)，初始化也明显掉帧
        > 渲染管线里面的这些Tile逻辑简单就是采样，可能被动态合批了，但也可以直接离线合批(一个节点搞定)
        > 纹理重复度问题，可以用一张较大的**边缘自拼接**底图纹理(1024*1024)，然后直接采样设置为重复就能有不错的纹理效果

      - 2. 实现方案：
        1. BG和内部TileMap + 边缘包边装饰，内部不进行TileMap的纹理映射，只有边缘根据周围9格选择不同的边缘纹理进行映射
        2. 为了简单分层级绘制：不同类型区域每个区域独立层次，包边独立层级，背景海水独立层级
        3. 每层只用一个与地图等大UI节点
        4. 通过shader自行采样绘制在UI节点上
           > 1. 内部实心部分，换算uv是校验是否在tileMap上决定是否丢弃
           > 2. 采样的时候保证纹理在节点上重复不缩放，直接放大uv倍数((tileSize*siezPerTile)/textSize)，让uv重复采样绘制
           >
           > 1. 将包边纹理进行合图以及控制间距，纹理的相对位置就是映射索引
           > 2. 校验uv是否在tileMap目标范围内，不再则丢弃
           > 3. 根据tileMap上的索引值计算对应合图纹理上的目标包边纹理偏移，再将uv换算到目标纹理的目标图样的uv上
           > 4. 具体换算,换算到tex实际坐标再换算：a= uv-ceil(tile_uv), b = tile_tex_size/map_tex_size,  off_tex_size; (a/b * tile_text_size + off_tex_size) / batch_tex_size
           > 
           > 1. 背景底图也是这种策略，但背景图多了水shader的渲染
           > 2. 水体效果一个是海绵波浪效果，一个是深度效果
           > 
...
- 渲染应用
  ...
  3. 水体/地表网格  
     前期用的Tile，但规模上去后有性能瓶颈，后面采取shader自己绘制方案，见地图/渲染相关细节
