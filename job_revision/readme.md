...
- 在线更新  
  ...
  > - import：重写的方法内直接import了方法题外import的模块的情况，重写的时候需要在函数题内import一遍，因为函数的定义是与模块有关的(py的顶层单位是模块，模块的import存在模块的global空间，函数对象定义的时候会直接引用这个global空间)，而重写的在线跟新函数是在另一个修补模块定义的，他无法直接在原函数内引用到原函数归属的模块，必须显式的再import一遍(但也不再该修补模块下import，防止多个在线更新冗余在一个模块下)    
