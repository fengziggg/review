...
- 性能优化[细节见](性能优化.md)：  
  - 性能差的表现：卡顿(怎么定义)，帧率低，网络时延，  
    - 归类：  
      - 逻辑类：业务实现异常：递归终止条件异常/业务优化：逻辑优化(冗余循环)，      
      - 资源类：资源精度，压缩，  
      - 平台性能瓶颈：C+/Py  
      - IO/计算压力：异步  
      - 多线程+并行拆分  
  - [优化实例](性能优化.md#优化实例)：UI，资源加载, scene，八叉树，批渲染，ECS，分块加载，复用滚动框架  
  - [思路](性能优化.md#排查手段)  
  - [工具](性能优化.md#排查手段)
  - 优化结果：  
- 内存占用：  
...
- 重点理解概念：
  ...
  3. 图集/合图/合批：[见详细描述](性能优化.md#图集合图合批断批原因)

  4. 压缩算法：[见性能分析篇中关于纹理的详细描述](性能优化.md#压缩纹理格式)
